...
''' <summary>
''' MARUZEN顧客固有 計上仕訳出力フォーム
''' Access版 fc_計上仕訳_MARUZEN 相当
''' FcJournalOutputBase を継承し、tw_fc_swk_wrk に計上仕訳を出力して CSV に書き込む。
''' kjkbn_id=1（費用）、rec_kbn IN(1,3) を対象。
''' 部署コード（一括用）を dr_bmn_cd に設定。未払金科目コードで cr_kmk_cd を上書き可能。
''' </summary>
Partial Public Class Form_fc_計上仕訳_MARUZEN
    ...
    Private Const KEY_SLIP_DT As String = "MARUZEN_KEIJOSWK_SLIP_DT"
    Private Const KEY_BUMON_CD As String = "MARUZEN_KEIJOSWK_BUMON_CD"
    Private Const KEY_NINSHONICHI As String = "MARUZEN_KEIJOSWK_NINSHONICHI"
    Private Const KEY_MIHARAIKIN_CD As String = "MARUZEN_KEIJOSWK_MIHARAIKIN_CD"
    Private Const KEY_GENSON_BUMON_CD As String = "MARUZEN_KEIJOSWK_GENSON_BUMON_CD"
    Private Const KEY_FOLDER As String = "MARUZEN_KEIJOSWK_FOLDER"
    Private Const KEY_FILE As String = "MARUZEN_KEIJOSWK_FILE"

    Private _settei As FcSetteiHelper

    Public Sub New()
        InitializeComponent()
        _settei = New FcSetteiHelper("MARUZEN")
    End Sub

    Private Sub Form_fc_計上仕訳_MARUZEN_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        Dim dtCount = _crud.GetDataTable(
            "SELECT COUNT(*) FROM tw_fc_swk_wrk WHERE customer_cd = 'MARUZEN' AND swk_kbn = '計上仕訳'")
        ...
        Dim dt = _crud.GetDataTable(
            "SELECT * FROM tw_fc_swk_wrk WHERE customer_cd = 'MARUZEN' AND swk_kbn = '計上仕訳' ORDER BY den_no, gyo_no")
    ...
    Private Sub cmd_選択2_Click(sender As Object, e As EventArgs) Handles cmd_選択2.Click
        Using dlg As New SaveFileDialog()
            dlg.Title = "出力先EXCELファイルを選択してください"
    ...
    ''' <summary>
    ''' tw_s_chuki_keijo → tw_fc_swk_wrk への INSERT SQL（MARUZEN計上仕訳版）
    ''' kjkbn_id=1（費用）、rec_kbn IN(1,3)。
    ''' 部署コード（一括用）を dr_bmn_cd に設定。未払金科目コードで cr_kmk_cd を上書き。
    ''' </summary>
    Protected Overrides Function BuildInsertToWrkSql(kikanFrom As Date) As String
        ...
        Dim bmnCd = txt_部署コード_一括用.Text.Replace("'", "''")
        Dim miharaiCd = txt_科目コード_未払金.Text.Replace("'", "''")

        Dim crKmkExpr = If(String.IsNullOrWhiteSpace(miharaiCd),
                           "COALESCE(h.cr_kmk_cd, '')",
                           $"CASE WHEN COALESCE(h.cr_kmk_cd, '') = '' THEN '{miharaiCd}' ELSE h.cr_kmk_cd END")
...
ORDER BY k.kykm_id"
    ...
    Protected Overrides Function WriteOutputFile(dt As DataTable, outputFolder As String) As String
        ...
        If Not String.IsNullOrWhiteSpace(txt_FileNM.Text) Then
            ...
            End If
        Else
            fileName = Path.Combine(outputFolder, $"MARUZEN_計上仕訳_{Format(Now, "yyyyMMdd")}.csv")
        End If
    ...
    Private Sub LoadSettings()
        ...
        txt_部署コード_一括用.Text = _settei.GetText(KEY_BUMON_CD)
        txt_承認日付.Text = _settei.GetText(KEY_NINSHONICHI)
        txt_科目コード_未払金.Text = _settei.GetText(KEY_MIHARAIKIN_CD)
        txt_部署コード_減損用.Text = _settei.GetText(KEY_GENSON_BUMON_CD)
    ...
    Private Sub SaveSettings()
        ...
        _settei.SetText(KEY_NINSHONICHI, txt_承認日付.Text)
        _settei.SetText(KEY_MIHARAIKIN_CD, txt_科目コード_未払金.Text)
        _settei.SetText(KEY_GENSON_BUMON_CD, txt_部署コード_減損用.Text)
        _settei.SetText(KEY_FOLDER, txt_FolderNM.Text)
        _settei.SetText(KEY_FILE, txt_FileNM.Text)
    End Sub

    Private Sub Form_fc_計上仕訳_MARUZEN_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _settei?.Dispose()
    End Sub
