...
''' <summary>
''' SNKO顧客 支払仕訳出力 条件フォーム
''' Access版 fc_SNKO_仕訳出力_JOKEN 相当
''' tw_fc_swk_wrk（SNKO 支払仕訳データ）の対象月・計上日・出力先を設定し、
''' 最終確認フォームへ渡す。
''' 固定値: 使用者社員番号・未払金科目コード
''' </summary>
Partial Public Class Form_fc_SNKO_仕訳出力_JOKEN
    ...
    Private Sub Form_fc_SNKO_仕訳出力_JOKEN_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LoadDefaults()
    End Sub

    Private Sub LoadDefaults()
        Try
            ' tw_fc_swk_wrk から支払月・計上日を取得
            Dim dt = _crud.GetDataTable(
                "SELECT den_date, shri_dt " &
                "FROM tw_fc_swk_wrk " &
            ...
            If dt IsNot Nothing AndAlso dt.Rows.Count > 0 Then
                ...
                If denDate.HasValue Then
                    txt_支払月.Text = denDate.Value.ToString("yyyy/MM")
    ...
    Private Sub UpdateOutputFileName()
        If Not String.IsNullOrWhiteSpace(txt_OUTPUT_FOLDER_NM.Text) AndAlso
           Not String.IsNullOrWhiteSpace(txt_支払月.Text) Then
            Dim month = txt_支払月.Text.Replace("/", "")
            Dim fileName = $"SNKO_仕訳_{month}.txt"
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        If dtCheck Is Nothing OrElse CInt(dtCheck.Rows(0)(0)) = 0 Then
            MessageBox.Show("出力対象の支払仕訳データがありません。", "確認", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If
        ...
        ' 最終確認フォームを開く
        Dim frmFinal As New Form_fc_SNKO_仕訳出力_最終確認()
        frmFinal.SetParams(txt_OUTPUT_FPATH.Text, txt_計上日.Text, txt_使用者社員番号.Text)
        frmFinal.ShowDialog(Me)
        Me.Close()
    End Sub

    Private Sub Form_fc_SNKO_仕訳出力_JOKEN_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _crud?.Dispose()
    End Sub
