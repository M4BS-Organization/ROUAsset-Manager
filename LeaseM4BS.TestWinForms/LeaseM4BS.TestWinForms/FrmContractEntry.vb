...
Public Class FrmContractEntry
    ' ▼▼▼ 1. クラスの先頭付近（変数の定義場所）に追加 ▼▼▼
    ' 一覧画面から受け取るためのID変数（プロパティ）
    Public Property TargetId As Double = 0
    ' ▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲▲
    ...
    Private Sub LoadMasters()
        ...
        ' -----------------------------------------------------
        Dim sqlKknri As String = "SELECT k.kknri_id, k.kknri1_cd, k.kknri1_nm, c.corp1_nm " &
                                     "FROM m_kknri k " &
                                     "LEFT JOIN m_corp c ON k.corp_id = c.corp_id " &
                                     "ORDER BY k.kknri1_cd"
        Dim dtKknri As DataTable = crud.GetDataTable(sqlKknri)

        cmb_KKNRI_ID.DataSource = dtKknri
        cmb_KKNRI_ID.ValueMember = "kknri_id"   ' 裏の値
        cmb_KKNRI_ID.DisplayMember = "kknri1_cd" ' 画面には「コード」だけ表示(Access仕様)
        cmb_KKNRI_ID.SelectedIndex = -1

        ' ドロップダウンの全体幅 (各列の合計+スクロールバー分)
        ' ★ここを変更
        cmb_KKNRI_ID.DropDownWidth = 600
        ' Fixed ではなく Variable (可変) にする
        cmb_KKNRI_ID.DrawMode = DrawMode.OwnerDrawVariable
        ' ここでの ItemHeight は「コンボボックス本体の高さ」になります (小さくする)
        cmb_KKNRI_ID.ItemHeight = 12
        ...
        Dim dtLcpt As DataTable = crud.GetDataTable(sqlLcpt)

        cmb_LCPT_ID.DataSource = dtLcpt
        cmb_LCPT_ID.ValueMember = "lcpt_id"
        cmb_LCPT_ID.DisplayMember = "lcpt1_cd" ' 画面にはコードだけ
        cmb_LCPT_ID.SelectedIndex = -1

        ' ★ここを変更
        cmb_LCPT_ID.DropDownWidth = 600
        cmb_LCPT_ID.DrawMode = DrawMode.OwnerDrawVariable
        cmb_LCPT_ID.ItemHeight = 12
    ...
    ' =========================================================
    Private Sub Combo_DrawItem(sender As Object, e As DrawItemEventArgs) Handles cmb_KKNRI_ID.DrawItem, cmb_LCPT_ID.DrawItem
        ...
        Dim w1 As Integer = 95  ' コード列
        Dim w2 As Integer = 265 ' 名称列
        Dim w3 As Integer = 150 ' 予備列(会社名等)

        ' 表示する文字を取得
        Dim text1 As String = "" ' コード
        Dim text2 As String = "" ' 名称
        Dim text3 As String = "" ' その他

        If cmb.Name = "cmb_KKNRI_ID" Then
            text1 = drv("kknri1_cd").ToString()
            text2 = drv("kknri1_nm").ToString()
            text3 = drv("corp1_nm").ToString() ' 会社名
        ElseIf cmb.Name = "cmb_LCPT_ID" Then
            w2 = 225 : w3 = 115 ' 支払先用の幅調整
            text1 = drv("lcpt1_cd").ToString()
            text2 = drv("lcpt1_nm").ToString()
            text3 = drv("lcpt2_nm").ToString() ' 支払先名2
        End If
        ...
        ' --- 1列目 (コード) ---
        Dim rect1 As New Rectangle(e.Bounds.X, e.Bounds.Y, w1, e.Bounds.Height)
        e.Graphics.DrawString(text1, e.Font, textBrush, rect1)
        e.Graphics.DrawLine(linePen, rect1.Right, rect1.Top, rect1.Right, rect1.Bottom) ' 縦線

        ' --- 2列目 (名称) ---
        Dim rect2 As New Rectangle(rect1.Right + 2, e.Bounds.Y, w2, e.Bounds.Height)
        e.Graphics.DrawString(text2, e.Font, textBrush, rect2)
        e.Graphics.DrawLine(linePen, rect2.Right, rect2.Top, rect2.Right, rect2.Bottom) ' 縦線

        ' --- 3列目 (その他) ---
        Dim rect3 As New Rectangle(rect2.Right + 2, e.Bounds.Y, w3, e.Bounds.Height)
        e.Graphics.DrawString(text3, e.Font, textBrush, rect3)
    ...
    Private Sub cmb_KKNRI_ID_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_KKNRI_ID.SelectedIndexChanged
        If cmb_KKNRI_ID.SelectedIndex = -1 Then
            ' クリア処理も Find を使う
            Dim found = Me.Controls.Find("txt_KKNRI_NM", True)
            If found.Length > 0 Then found(0).Text = ""

            Dim foundC = Me.Controls.Find("txt_CORP_NM", True)
            If foundC.Length > 0 Then foundC(0).Text = ""
            Return
        End If

        Dim drv As DataRowView = TryCast(cmb_KKNRI_ID.SelectedItem, DataRowView)
        If drv IsNot Nothing Then
            ' ★修正: Findの結果(0番目)を直接使う
            Dim foundK = Me.Controls.Find("txt_KKNRI_NM", True)
            If foundK.Length > 0 Then
                foundK(0).Text = drv("kknri1_nm").ToString()
            End If

            Dim foundC = Me.Controls.Find("txt_CORP_NM", True)
            If foundC.Length > 0 Then
                foundC(0).Text = drv("corp1_nm").ToString()
            End If
        End If
    End Sub
    ...
    Private Sub cmb_LCPT_ID_SelectedIndexChanged(sender As Object, e As EventArgs) Handles cmb_LCPT_ID.SelectedIndexChanged
        If cmb_LCPT_ID.SelectedIndex = -1 Then
            Dim found = Me.Controls.Find("txt_LCPT_NM", True)
            If found.Length > 0 Then found(0).Text = ""
            Return
        End If

        Dim drv As DataRowView = TryCast(cmb_LCPT_ID.SelectedItem, DataRowView)
        If drv IsNot Nothing Then
            ' ★修正: Findの結果(0番目)を直接使う
            Dim foundL = Me.Controls.Find("txt_LCPT_NM", True)
            If foundL.Length > 0 Then
                foundL(0).Text = drv("lcpt1_nm").ToString()
            End If
        End If
    End Sub
    ...
    Private Sub BindCombo(cmb As ComboBox, sql As String, display As String, value As String)
    ...
    Private Sub CalculateAmount()
        ' 数値変換
        Dim price As Decimal = ToDecimal(txt_KNYUKN.Text)
        Dim rate As Decimal = ToDecimal(txt_RYORITU.Text)

        If price > 0 And rate > 0 Then
            ' 月額 = 購入価額 * 料率 (Accessロジック: 切り捨て)
            Dim monthly As Decimal = Math.Floor(price * rate)
            txt_GLSRYO.Text = monthly.ToString("#,##0")

            ' 消費税計算
            Dim taxRate As Decimal = ToDecimal(cmb_ZRITU.Text)
            Dim tax As Decimal = Math.Floor(monthly * taxRate)

            txt_GZEI.Text = tax.ToString("#,##0") ' 月額消費税
            txt_GLSRYO_ZEIKOMI.Text = (monthly + tax).ToString("#,##0") ' 税込み
        End If
    End Sub
    ...
    Private Sub cmd_TOUROKU_Click(sender As Object, e As EventArgs) Handles cmd_TOUROKU.Click
        ...
        Dim currentKykhId As Double = ToDecimal(txt_KYKH_ID.Text)
        ...
        Try
            ...
            If isNewMode Then
                ...
                currentKykhId = Convert.ToDouble(crud.ExecuteScalar(Of Object)(sqlMaxId)) + 1
            ...
            ' --- 金額 ---
            valKykh.Add("k_knyukn", ToDecimal(txt_KNYUKN.Text))
            valKykh.Add("ryoritu", ToDecimal(txt_RYORITU.Text))
            valKykh.Add("k_glsryo", ToDecimal(txt_GLSRYO.Text))
            valKykh.Add("zritu", ToDecimal(cmb_ZRITU.Text))
            valKykh.Add("k_gzei", ToDecimal(txt_GZEI.Text))
            valKykh.Add("k_glsryo_zkomi", ToDecimal(txt_GLSRYO_ZEIKOMI.Text))

            ' --- 必須項目の穴埋め ---
            valKykh.Add("saikaisu", 0)
            valKykh.Add("lkikan", ToDecimal(txt_LKIKAN.Text)) ' 画面値を使う
            valKykh.Add("shri_kn", ToDecimal(txt_SHRI_KN.Text))
            valKykh.Add("shri_cnt", ToDecimal(txt_SHRI_CNT.Text))
            valKykh.Add("k_suuryo", 0)
            valKykh.Add("k_klsryo", ToDecimal(txt_KLSRYO.Text))
            valKykh.Add("k_mlsryo", ToDecimal(txt_MLSRYO.Text))
            valKykh.Add("k_slsryo", ToDecimal(txt_SLSRYO.Text))
            valKykh.Add("k_kzei", ToDecimal(txt_KZEI.Text))
            valKykh.Add("k_mzei", ToDecimal(txt_MZEI.Text))
            valKykh.Add("k_klsryo_zkomi", ToDecimal(txt_KLSRYO_ZKOMI.Text))
            valKykh.Add("k_mlsryo_zkomi", ToDecimal(txt_MLSRYO_ZKOMI.Text))
            ...
            ' --- システム項目 ---
            ' ▼▼▼ ★この1行を追加してください！ ▼▼▼
            valKykh.Add("update_cnt", ToDecimal(txt_UPDATE_CNT.Text))
            ...
            Dim nextKykmId As Double = Convert.ToDouble(crud.ExecuteScalar(Of Object)(sqlMaxDetailId)) + 1
            ...
            For Each row As DataGridViewRow In dgv_DETAILS.Rows
                ...
                ' ※なければ 0 になる
                Dim rowKykmId As Double = ToDecimal(row.Cells("col_KYKM_ID").Value)
                ...
                valKykm.Add("b_suuryo", ToDecimal(row.Cells("b_suuryo").Value))
                valKykm.Add("b_knyukn", ToDecimal(row.Cells("b_knyukn").Value))
                valKykm.Add("b_klsryo", ToDecimal(row.Cells("b_klsryo").Value))
                valKykm.Add("b_glsryo", ToDecimal(row.Cells("b_glsryo").Value))
                ...
                Else
                    ...
                    pList.Add(New Npgsql.NpgsqlParameter("@su", ToDecimal(row.Cells(1).Value)))
                    pList.Add(New Npgsql.NpgsqlParameter("@kn", ToDecimal(row.Cells(3).Value)))
                    pList.Add(New Npgsql.NpgsqlParameter("@kl", ToDecimal(row.Cells(4).Value)))
                    pList.Add(New Npgsql.NpgsqlParameter("@gl", ToDecimal(row.Cells(5).Value)))
                    pList.Add(New Npgsql.NpgsqlParameter("@upd", DateTime.Now))
                    pList.Add(New Npgsql.NpgsqlParameter("@id", rowKykmId))

                    crud.ExecuteNonQuery(updSql, pList)
                End If
                detailNo += 1
            Next
    ...
    Private Sub LoadContractData(kyakNo As String)
        ...
        Try
            ...
            Dim sqlHead As String = "SELECT * FROM d_kykh WHERE TRIM(kykbnj) = @no ORDER BY kykh_id DESC LIMIT 1"

            Dim prmHead As New List(Of Npgsql.NpgsqlParameter) From {
                New Npgsql.NpgsqlParameter("@no", kyakNo)
            }
            Dim dtHead As DataTable = crud.GetDataTable(sqlHead, prmHead)

            ' デバッグ用: 何件見つかったか表示（確認後、削除してOK）
            ' MessageBox.Show("検索SQL実行結果: " & dtHead.Rows.Count & "件 見つかりました。")
            ...
            txt_KYKH_ID.Text = row("kykh_id").ToString()
            txt_UPDATE_CNT.Text = row("update_cnt").ToString()

            ' 日付 (DBがNULLでなければセット)
            If Not IsDBNull(row("kyak_dt")) Then dtp_KYAK_DT.Value = Convert.ToDateTime(row("kyak_dt"))
            If Not IsDBNull(row("start_dt")) Then dtp_START_DT.Value = Convert.ToDateTime(row("start_dt"))
            If Not IsDBNull(row("end_dt")) Then dtp_END_DT.Value = Convert.ToDateTime(row("end_dt"))

            ' 金額・数値
            txt_KNYUKN.Text = ToDecimal(row("k_knyukn")).ToString("#,##0")
            txt_RYORITU.Text = ToDecimal(row("ryoritu")).ToString("0.000000")
            txt_GLSRYO.Text = ToDecimal(row("k_glsryo")).ToString("#,##0")
            cmb_ZRITU.Text = ToDecimal(row("zritu")).ToString("0.00")
            txt_GZEI.Text = ToDecimal(row("k_gzei")).ToString("#,##0")
            txt_GLSRYO_ZEIKOMI.Text = ToDecimal(row("k_glsryo_zkomi")).ToString("#,##0")
            ...
            Dim sqlDetail As String = "SELECT * FROM d_kykm WHERE kykh_id = @id ORDER BY kykm_no"
            ' パラメータ名が違うので新しく作る
            Dim prmDetail As New List(Of Npgsql.NpgsqlParameter) From {
                New Npgsql.NpgsqlParameter("@id", kykhId)
            }
            Dim dtDetail As DataTable = crud.GetDataTable(sqlDetail, prmDetail)
            ...
            For Each dRow As DataRow In dtDetail.Rows
                dgv_DETAILS.Rows.Add(
                    dRow("kykm_id"),    ' 隠しID
                    dRow("bukn_nm"),    ' 物件名
                    ToDecimal(dRow("b_suuryo")).ToString("#,##0"),   ' 数量
                    "",                 ' 管理部署
                    ToDecimal(dRow("b_knyukn")).ToString("#,##0"),   ' 購入価額
                    ToDecimal(dRow("b_klsryo")).ToString("#,##0"),   ' 1支払額
                    ToDecimal(dRow("b_glsryo")).ToString("#,##0")    ' 月額
                )
            Next
    ...
    Private Sub cmd_REVISE_Click(sender As Object, e As EventArgs) Handles cmd_REVISE.Click
        ...
        Try
            ...
            ' ★UPDATE実行
            crud.Update("d_kykh", valKykh, "kykh_id = " & targetId)
            ...
            crud.ExecuteNonQuery(sqlDel, prmDel)

            ' B. 新しい明細IDの採番準備
            Dim sqlMaxDetailId As String = "SELECT COALESCE(MAX(kykm_id), 0) FROM d_kykm"
            Dim currentKykmId As Double = Convert.ToDouble(crud.ExecuteScalar(Of Object)(sqlMaxDetailId))
            ...
            For Each row As DataGridViewRow In dgv_DETAILS.Rows
                ...
                valKykm.Add("b_suuryo", ToDecimal(row.Cells(1).Value))
                valKykm.Add("b_knyukn", ToDecimal(row.Cells(3).Value))
                valKykm.Add("b_klsryo", ToDecimal(row.Cells(4).Value))
                valKykm.Add("b_glsryo", ToDecimal(row.Cells(5).Value))
    ...
    Private Sub cmd_DELETE_Click(sender As Object, e As EventArgs) Handles cmd_DELETE.Click
        ...
        Try
            ...
            crud.ExecuteNonQuery(sqlDelDetail, prm)

            ' B. ヘッダ (d_kykh) の削除
            Dim sqlDelHead As String = "DELETE FROM d_kykh WHERE kykh_id = @id"
            ' パラメータは同じものが使えるので再利用
            crud.ExecuteNonQuery(sqlDelHead, prm)
    ...
    Private Sub FrmContractEntry_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _crud.Dispose()
    End Sub
    ...
    Private Function ValidateInput() As Boolean
        ...
        If ToDecimal(txt_GLSRYO.Text) = 0 Then
    ...
    ' =========================================================
    Public Sub LoadContractById(id As Double)
        ...
        Try
            ' --- ヘッダ取得 (ID指定) ---
            Dim sqlHead As String = "SELECT * FROM d_kykh WHERE kykh_id = @id"
            Dim prmHead As New List(Of Npgsql.NpgsqlParameter) From {
            ...
            If dtHead.Rows.Count > 0 Then
                ...
                txt_KYAK_NO.Text = row("kykbnj").ToString() ' 契約番号

                ' ▼▼▼ ★追加: 読み込み時もここが抜けていました ▼▼▼
                txt_KYAK_NM.Text = row("kyak_nm").ToString()      ' 契約名
                txt_RNG_BANGO.Text = row("rng_bango").ToString()  ' 稟議番号
                txt_JISH_KYAK_NO.Text = row("kykbnl").ToString()  ' 自社管理用
                ' 日付
                If Not IsDBNull(row("kyak_dt")) Then dtp_KYAK_DT.Value = Convert.ToDateTime(row("kyak_dt"))
                If Not IsDBNull(row("start_dt")) Then dtp_START_DT.Value = Convert.ToDateTime(row("start_dt"))
                If Not IsDBNull(row("end_dt")) Then dtp_END_DT.Value = Convert.ToDateTime(row("end_dt"))
                ...
                ' 金額
                txt_KNYUKN.Text = row("k_knyukn").ToString()
                txt_RYORITU.Text = row("ryoritu").ToString()
                txt_GLSRYO.Text = row("k_glsryo").ToString()
                cmb_ZRITU.Text = row("zritu").ToString()
                txt_GZEI.Text = row("k_gzei").ToString()
                txt_GLSRYO_ZEIKOMI.Text = row("k_glsryo_zkomi").ToString()
                ...
                ' --- 明細取得 (ID指定) ---
                Dim sqlDetail As String = "SELECT * FROM d_kykm WHERE kykh_id = @id ORDER BY kykm_no"
                Dim dtDetail As DataTable = crud.GetDataTable(sqlDetail, prmHead) ' パラメータは同じ@idでOK
                ...
                For Each dRow As DataRow In dtDetail.Rows
                    ...
                    dgv_DETAILS.Rows.Add(
                        ...
                        ToDecimal(dRow("b_suuryo")).ToString("#,##0"),   ' Index 2: 数量
                        "",                                              ' Index 3: 管理部署
                        ToDecimal(dRow("b_knyukn")).ToString("#,##0"),   ' Index 4: 購入価額
                        ToDecimal(dRow("b_klsryo")).ToString("#,##0"),   ' Index 5: 1支払額
                        ToDecimal(dRow("b_glsryo")).ToString("#,##0")    ' Index 6: 月額
                    )
                Next
            End If
    ...
    Private Function ToDecimal(val As Object) As Decimal
        ...
        If Decimal.TryParse(val.ToString().Replace(",", ""), result) Then
            Return result
        End If
        Return 0
    End Function
    ...
    Private Sub CalcTaxAndTotal(txtAmount As TextBox, txtTax As TextBox, txtTotal As TextBox)
        ' 1. 金額と税率を取得
        Dim amount As Decimal = ToDecimal(txtAmount.Text)
        Dim rate As Decimal = ToDecimal(cmb_ZRITU.Text) ' コンボボックスの表示値(0.10など)を使用
        ...
        ' 3. 結果を表示 (3桁カンマ区切り)
        txtAmount.Text = amount.ToString("#,##0") ' 入力値も整形する
        txtTax.Text = tax.ToString("#,##0")
    ...
    Private Sub CalcPaymentInfo()
        ...
        ' 1. 支払回数の計算 (期間 ÷ 間隔)
        ' -----------------------------------------------------
        Dim kikan As Decimal = ToDecimal(txt_LKIKAN.Text) ' 契約期間
        Dim kankaku As Decimal = ToDecimal(txt_SHRI_KN.Text) ' 支払間隔
        ...
        ' -----------------------------------------------------
        Dim monthlyFee As Decimal = ToDecimal(txt_GLSRYO.Text) ' 月額
        ...
        ' 3. 総額リース料の計算
        ' -----------------------------------------------------
        Dim prepayCount As Decimal = ToDecimal(txt_MKAISU.Text) ' 前払回数
        Dim prepayAmount As Decimal = ToDecimal(txt_MLSRYO.Text) ' 前払金
        ...
        If txtTotalLease.Length > 0 Then
            txtTotalLease(0).Text = totalLease.ToString("#,##0")
        End If
    End Sub
    ' ---------------------------------------------------------
    ' イベント: 値が変わったら再計算
