...
''' <summary>
''' VALQUA顧客固有 支払仕訳出力フォーム
''' Access版 fc_VALQUA_支払仕訳 相当
''' FcJournalOutputBase を継承し、tw_fc_swk_wrk に支払仕訳を出力して複数CSVに書き込む。
''' kjkbn_id=1（費用）、rec_kbn IN(1,3) を対象。
''' 未払/未収/債務取崩の3ファイルをチェックボックスに応じて出力。
''' </summary>
Partial Public Class Form_fc_VALQUA_支払仕訳
    ...
    Private Const KEY_KEIJYO_DT As String = "VALQUA_HARAISWK_KEIJYO_DT"
    Private Const KEY_FOLDER As String = "VALQUA_HARAISWK_FOLDER"
    Private Const KEY_FNAME_MIHARAI As String = "VALQUA_HARAISWK_FNAME_MIHARAI"
    Private Const KEY_FNAME_MISHUU As String = "VALQUA_HARAISWK_FNAME_MISHUU"
    Private Const KEY_FNAME_SAIMU As String = "VALQUA_HARAISWK_FNAME_SAIMU"
    ...
    Private Sub Form_fc_VALQUA_支払仕訳_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        chk_未払F.Checked = True
        chk_未収F.Checked = True
...
ORDER BY k.kykm_id"
        ...
        Dim dtCount = _crud.GetDataTable(
            "SELECT COUNT(*) FROM tw_fc_swk_wrk WHERE customer_cd = 'VALQUA' AND swk_kbn = '支払仕訳'")
        ...
        Dim dt = _crud.GetDataTable(
            "SELECT * FROM tw_fc_swk_wrk WHERE customer_cd = 'VALQUA' AND swk_kbn = '支払仕訳' ORDER BY den_no, gyo_no")

        Dim results As New List(Of String)
        If chk_未払F.Checked AndAlso Not String.IsNullOrWhiteSpace(txt_FNAME_未払.Text) Then
            results.Add(WriteCsvFile(dt, txt_FOLDER.Text, txt_FNAME_未払.Text))
        End If
        If chk_未収F.Checked AndAlso Not String.IsNullOrWhiteSpace(txt_FNAME_未収.Text) Then
            results.Add(WriteCsvFile(dt, txt_FOLDER.Text, txt_FNAME_未収.Text))
        End If
    ...
    ''' <summary>VALQUA支払仕訳は BuildInsertToWrkSql を使わず cmd_実行_Click 内で直接SQL実行。</summary>
    ...
    Private Sub LoadSettings()
        ...
        txt_FNAME_未払.Text = _settei.GetText(KEY_FNAME_MIHARAI)
        txt_FNAME_未収.Text = _settei.GetText(KEY_FNAME_MISHUU)
    ...
    Private Sub SaveSettings()
        ...
        _settei.SetText(KEY_FNAME_MIHARAI, txt_FNAME_未払.Text)
        _settei.SetText(KEY_FNAME_MISHUU, txt_FNAME_未収.Text)
        _settei.SetText(KEY_FNAME_SAIMU, txt_FNAME_債務取崩.Text)
    End Sub

    Private Sub Form_fc_VALQUA_支払仕訳_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _settei?.Dispose()
    End Sub
