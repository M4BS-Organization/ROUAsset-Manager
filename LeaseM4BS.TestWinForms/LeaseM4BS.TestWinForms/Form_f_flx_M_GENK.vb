...
Partial Public Class Form_f_flx_M_GENK
    ...
    Private Sub Form_f_flx_M_GENK_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SearchData()
    ...
    Private Function BuildSql(searchText As String, ByRef prms As List(Of NpgsqlParameter))
        ...
        sb.AppendLine("  genk_cd AS 原価区分コード, ")
        sb.AppendLine("  genk_nm AS 原価区分名, ")
        sb.AppendLine("  biko AS 備考, ")
        sb.AppendLine("  create_dt AS 作成日時, ")
        sb.AppendLine("  update_dt AS 更新日時, ")
        sb.AppendLine("  genk_id AS ID")

        sb.AppendLine("FROM m_genk ")
        sb.AppendLine("WHERE genk_id <> 0 ")

        ' --- 検索条件 (WHERE) ---
        If txt_SEARCH.Text.Trim() <> "" Then
            sb.AppendLine("AND genk_cd LIKE @search ")
            prms.Add(New NpgsqlParameter("@search", $"%{searchText}%"))
        End If

        sb.AppendLine("ORDER BY genk_id;")
