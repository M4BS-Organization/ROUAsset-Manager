...
''' <summary>
''' TSYSCOM顧客固有 支払仕訳出力フォーム
''' Access版 fc_TSYSCOM_支払仕訳 相当
...
Partial Public Class Form_fc_TSYSCOM_支払仕訳
    ...
    Private Const KEY_SLIP_DT As String = "TSYSCOM_HARAISWK_SLIP_DT"
    Private Const KEY_FOLDER As String = "TSYSCOM_HARAISWK_OUTPUT_FOLDER1"
    Private Const KEY_FILE2 As String = "TSYSCOM_HARAISWK_OUTPUT_FILE2"
    ...
    Private Sub Form_fc_TSYSCOM_支払仕訳_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        txt_KTMG.ReadOnly = True
        txt_YMD_02_曜日.ReadOnly = True
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        Dim dtCount = _crud.GetDataTable(
            "SELECT COUNT(*) FROM tw_fc_swk_wrk WHERE customer_cd = 'TSYSCOM' AND swk_kbn = '支払仕訳'")
        ...
        Dim dt = _crud.GetDataTable(
            "SELECT * FROM tw_fc_swk_wrk WHERE customer_cd = 'TSYSCOM' AND swk_kbn = '支払仕訳' ORDER BY den_no, gyo_no")
    ...
    ''' <summary>
    ''' tw_s_chuki_keijo → tw_fc_swk_wrk への INSERT SQL（TSYSCOM支払仕訳版）
...
ORDER BY k.kykm_id"
    ...
    Protected Overrides Function WriteOutputFile(dt As DataTable, outputFolder As String) As String
        Dim fileName = Path.Combine(outputFolder, $"TSYSCOM_支払仕訳_{Format(Now, "yyyyMMdd")}.csv")
    ...
    Private Sub Form_fc_TSYSCOM_支払仕訳_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _settei?.Dispose()
    End Sub
