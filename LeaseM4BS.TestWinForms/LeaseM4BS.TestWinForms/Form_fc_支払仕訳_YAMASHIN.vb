...
''' <summary>
''' YAMASHIN顧客固有 支払仕訳出力フォーム
''' Access版 fc_支払仕訳_YAMASHIN 相当
''' FcJournalOutputBase を継承し、tw_fc_swk_wrk に支払仕訳を出力して CSV に書き込む。
''' kjkbn_id=1（費用）、rec_kbn IN(1,3) を対象。
''' KAMOKU_CD_01: "14110"（出金口座 一括用）
''' KAMOKU_CD_02: "31300"（翌期分 売買 leakbn_id=1）
''' KAMOKU_CD_03: "18100"（翌期分 賃貸 leakbn_id=3,4）
''' chk_CHK_01: 既存データ上書き確認フラグ
''' </summary>
Partial Public Class Form_fc_支払仕訳_YAMASHIN
    ...
    Private Const KEY_SLIP_DT As String = "YAMASHIN_HARAISWK_SLIP_DT"
    Private Const KEY_KAMOKU_01 As String = "YAMASHIN_HARAISWK_KAMOKU_CD_01"
    Private Const KEY_KAMOKU_02 As String = "YAMASHIN_HARAISWK_KAMOKU_CD_02"
    Private Const KEY_KAMOKU_03 As String = "YAMASHIN_HARAISWK_KAMOKU_CD_03"
    Private Const KEY_FOLDER1 As String = "YAMASHIN_HARAISWK_OUTPUT_FOLDER1"
    ...
    Private Sub Form_fc_支払仕訳_YAMASHIN_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        Dim dtCount = _crud.GetDataTable(
            "SELECT COUNT(*) FROM tw_fc_swk_wrk WHERE customer_cd = 'YAMASHIN' AND swk_kbn = '支払仕訳'")
        ...
        Dim dt = _crud.GetDataTable(
            "SELECT * FROM tw_fc_swk_wrk WHERE customer_cd = 'YAMASHIN' AND swk_kbn = '支払仕訳' ORDER BY den_no, gyo_no")
    ...
    ''' <summary>
    ''' tw_s_chuki_keijo → tw_fc_swk_wrk への INSERT SQL（YAMASHIN支払仕訳版）
    ''' kjkbn_id=1（費用）、rec_kbn IN(1,3)。
    ''' 翌月以降は leakbn_id に応じて KAMOKU_CD_02（売買）または KAMOKU_CD_03（賃貸）を使用。
    ''' </summary>
    Protected Overrides Function BuildInsertToWrkSql(kikanFrom As Date) As String
        ...
        Dim kmkCd02 = txt_KAMOKU_CD_02.Text  ' 翌期分（売買 leakbn_id=1）
        Dim kmkCd03 = txt_KAMOKU_CD_03.Text  ' 翌期分（賃貸 leakbn_id=3,4）
...
SELECT
    ...
    CASE
        ...
        ELSE CASE WHEN k.leakbn_id = 1 THEN '{kmkCd02}' ELSE '{kmkCd03}' END
    END AS cr_kmk_cd,
    CASE
        WHEN DATE_TRUNC('month', k.shri_dt) = DATE_TRUNC('month', '{kikanFromStr}'::date)
...
ORDER BY k.kykm_id"
    ...
    Protected Overrides Function WriteOutputFile(dt As DataTable, outputFolder As String) As String
        Dim fileName = Path.Combine(outputFolder, $"YAMASHIN_支払仕訳_{Format(Now, "yyyyMMdd")}.csv")
    ...
    Private Sub LoadSettings()
        ...
        txt_KAMOKU_CD_01.Text = _settei.GetText(KEY_KAMOKU_01, "14110")
        txt_KAMOKU_CD_02.Text = _settei.GetText(KEY_KAMOKU_02, "31300")
        txt_KAMOKU_CD_03.Text = _settei.GetText(KEY_KAMOKU_03, "18100")
    ...
    Private Sub Form_fc_支払仕訳_YAMASHIN_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _settei?.Dispose()
    End Sub
