...
Partial Public Class Form_fc_計上仕訳_KYOTO
    ...
    Private Const KEY_SLIP_DT As String = "KYOTO_KEIJOSWK_SLIP_DT"
    Private Const KEY_FOLDER As String = "KYOTO_KEIJOSWK_OUTPUT_FOLDER"
    ...
    Private Sub Form_fc_計上仕訳_KYOTO_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        Dim dtCount = _crud.GetDataTable(
            "SELECT COUNT(*) FROM tw_fc_swk_wrk WHERE customer_cd = 'KYOTO' AND swk_kbn = '計上仕訳'")
        ...
        Dim dt = _crud.GetDataTable(
            "SELECT * FROM tw_fc_swk_wrk WHERE customer_cd = 'KYOTO' AND swk_kbn = '計上仕訳' ORDER BY den_no, gyo_no")
    ...
    ''' <summary>
    ''' tw_s_chuki_keijo → tw_fc_swk_wrk への INSERT SQL（KYOTO計上仕訳版）
...
ORDER BY kykm_id, rec_kbn"
    ...
    Protected Overrides Function WriteOutputFile(dt As DataTable, outputFolder As String) As String
        Dim fileName = Path.Combine(outputFolder, $"KYOTO_計上仕訳_{Format(Now, "yyyyMMdd")}.csv")
    ...
    Private Sub Form_fc_計上仕訳_KYOTO_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _settei?.Dispose()
    End Sub
