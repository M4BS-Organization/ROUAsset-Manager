...
Partial Public Class Form_f_flx_M_HKMK
    ...
    Private Sub Form_f_flx_M_HKMK_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SearchData()
    End Sub
    ...
    Private Function BuildSql(searchText As String, ByRef prms As List(Of NpgsqlParameter))
        ...
        sb.AppendLine("  hkmk_cd AS 費用区分コード, ")
        sb.AppendLine("  hkmk_nm AS 費用区分名, ")
        sb.AppendLine("  sum1_cd AS 集計区分1コード, ")
        sb.AppendLine("  sum1_nm AS 集計区分1, ")
        sb.AppendLine("  sum2_cd AS 集計区分2コード, ")
        sb.AppendLine("  sum2_nm AS 集計区分2, ")
        sb.AppendLine("  sum3_cd AS 集計区分3コード, ")
        sb.AppendLine("  sum3_nm AS 集計区分3, ")
        sb.AppendLine("  hrel_ptn_cd3 AS 費用決定要素コード, ")
        sb.AppendLine("  hrel_ptn_nm3 AS 費用決定要素, ")
        sb.AppendLine("  create_dt AS 作成日時, ")
        sb.AppendLine("  update_dt AS 更新日時, ")
        sb.AppendLine("  hkmk_id AS ID")

        sb.AppendLine("FROM m_hkmk ")
        sb.AppendLine("WHERE hkmk_id <> 0 ")

        ' --- 検索条件 (WHERE) ---
        If txt_SEARCH.Text.Trim() <> "" Then
        Else
            sb.AppendLine("AND hkmk_cd LIKE @search OR sum1_cd LIKE @search OR sum2_cd LIKE @search OR sum3_cd LIKE @search ")
            prms.Add(New NpgsqlParameter("@search", $"%{searchText}%"))
        End If

        sb.AppendLine("ORDER BY hkmk_id;")
    ...
    Private Sub cmd_NEW_Click(sender As Object, e As EventArgs) Handles cmd_NEW.Click
        Dim frm As New Form_f_M_HKMK_INP
        frm.ShowDialog()
    ...
    Private Sub cmd_CHANGE_Click(sender As Object, e As EventArgs) Handles cmd_CHANGE.Click
        ...
        Dim frm As New Form_f_M_HKMK_CHANGE
        frm.HkmkId = Convert.ToDouble(selectedRow.Cells("id").Value)
        frm.ShowDialog()
