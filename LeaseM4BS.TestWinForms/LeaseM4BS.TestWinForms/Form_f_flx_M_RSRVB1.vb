...
Partial Public Class Form_f_flx_M_RSRVB1
    ...
    Private Function BuildSql(searchText As String, ByRef prms As List(Of NpgsqlParameter))
        ...
        sb.AppendLine("  rsrvb1_cd AS 予備コード, ")
        sb.AppendLine("  rsrvb1_nm AS 予備名, ")
        sb.AppendLine("  biko AS 備考, ")
        sb.AppendLine("  create_dt AS 作成日時, ")
        sb.AppendLine("  update_dt AS 更新日時, ")
        sb.AppendLine("  rsrvb1_id AS ID")

        sb.AppendLine("FROM m_rsrvb1 ")
        sb.AppendLine("WHERE rsrvb1_id <> 0 ")

        ' --- 検索条件 (WHERE) ---
        If txt_SEARCH.Text.Trim() <> "" Then
        Else
            sb.AppendLine("AND rsrvb1_cd LIKE @search ")
            prms.Add(New NpgsqlParameter("@search", $"%{searchText}%"))
        End If

        sb.AppendLine("ORDER BY rsrvb1_id;")
