...
Partial Public Class Form_f_flx_M_RSRVH1
    ...
    Private Function BuildSql(searchText As String, ByRef prms As List(Of NpgsqlParameter))
        ...
        sb.AppendLine("  rsrvh1_cd AS 予備コード, ")
        sb.AppendLine("  rsrvh1_nm AS 予備名, ")
        sb.AppendLine("  biko AS 備考, ")
        sb.AppendLine("  create_dt AS 作成日時, ")
        sb.AppendLine("  update_dt AS 更新日時, ")
        sb.AppendLine("  rsrvh1_id AS ID")

        sb.AppendLine("FROM m_rsrvh1 ")
        sb.AppendLine("WHERE rsrvh1_id <> 0 ")

        ' --- 検索条件 (WHERE) ---
        ' テキストボックスに入力があれば、業者コードで検索
        If txt_SEARCH.Text.Trim() <> "" Then
            sb.AppendLine("AND rsrvh1_cd LIKE @search ")
            prms.Add(New NpgsqlParameter("@search", $"%{searchText}%"))
        End If

        sb.AppendLine("ORDER BY rsrvh1_id;")
