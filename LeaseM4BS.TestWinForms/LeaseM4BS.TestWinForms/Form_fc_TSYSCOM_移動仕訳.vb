...
''' <summary>
''' TSYSCOM顧客固有 移動仕訳出力フォーム
''' Access版 fc_TSYSCOM_移動仕訳 相当
''' FcJournalOutputBase を継承し、tw_fc_swk_wrk に移動仕訳を出力して CSV に書き込む。
''' kjkbn_id=2（資産）、rec_kbn IN(1,3) および rec_kbn=2（zei>0）を対象。
''' chk_CHK_01: 既存データ上書き確認フラグ
''' </summary>
Partial Public Class Form_fc_TSYSCOM_移動仕訳
    ...
    Private Const KEY_SLIP_DT As String = "TSYSCOM_IDOSWK_SLIP_DT"
    Private Const KEY_FOLDER As String = "TSYSCOM_IDOSWK_OUTPUT_FOLDER1"
    Private Const KEY_FILE2 As String = "TSYSCOM_IDOSWK_OUTPUT_FILE2"
    ...
    Private Sub Form_fc_TSYSCOM_移動仕訳_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        Dim dtCount = _crud.GetDataTable(
            "SELECT COUNT(*) FROM tw_fc_swk_wrk WHERE customer_cd = 'TSYSCOM' AND swk_kbn = '移動仕訳'")
        ...
        Dim dt = _crud.GetDataTable(
            "SELECT * FROM tw_fc_swk_wrk WHERE customer_cd = 'TSYSCOM' AND swk_kbn = '移動仕訳' ORDER BY den_no, gyo_no")
    ...
    ''' <summary>
    ''' tw_s_chuki_keijo → tw_fc_swk_wrk への INSERT SQL（TSYSCOM移動仕訳版）
...
ORDER BY kykm_id, rec_kbn"
    ...
    Protected Overrides Function WriteOutputFile(dt As DataTable, outputFolder As String) As String
        Dim fileName = Path.Combine(outputFolder, $"TSYSCOM_移動仕訳_{Format(Now, "yyyyMMdd")}.csv")
    ...
    Private Sub Form_fc_TSYSCOM_移動仕訳_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _settei?.Dispose()
    End Sub
