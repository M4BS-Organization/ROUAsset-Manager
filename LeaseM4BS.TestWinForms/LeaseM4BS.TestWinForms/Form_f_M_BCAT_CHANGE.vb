...
Partial Public Class Form_f_M_BCAT_CHANGE
    ...
    Private Sub Form_f_M_BCAT_CHANGE_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            ' --- ヘッダ取得 (ID指定) ---
            Dim sqlHead As String = "SELECT * FROM m_bcat " &
            ...
            If dtHead.Rows.Count > 0 Then
                ...
                ' 画面項目に値をセット
                txt_BCAT1_CD.Text = row("bcat1_cd").ToString()
                txt_BCAT1_NM.Text = row("bcat1_nm").ToString()
                cmb_BCAT2_CD.SelectedValue = row("bcat2_cd").ToString()
                txt_BCAT2_NM.Text = row("bcat2_nm").ToString()
                cmb_BCAT3_CD.SelectedValue = row("bcat3_cd").ToString()
                txt_BCAT3_NM.Text = row("bcat3_nm").ToString()
                cmb_BCAT4_CD.SelectedValue = row("bcat4_cd").ToString()
                txt_BCAT4_NM.Text = row("bcat4_nm").ToString()
                cmb_BCAT5_CD.SelectedValue = row("bcat5_cd").ToString()
                txt_BCAT5_NM.Text = row("bcat5_nm").ToString()
                ...
                txt_BKNRI_NM.Text = row("bknri1_nm").ToString()
                ...
                txt_BCAT_ID.Text = row("bcat_id").ToString()
            End If
    ...
    Private Sub cmd_CREATE_Click(sender As Object, e As EventArgs) Handles cmd_CREATE.Click
        ...
        prms.Add(New NpgsqlParameter("@id", Integer.Parse(txt_BCAT_ID.Text)))
