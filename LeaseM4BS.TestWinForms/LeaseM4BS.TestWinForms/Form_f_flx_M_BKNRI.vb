...
Partial Public Class Form_f_flx_M_BKNRI
    ...
    Private Sub Form_f_flx_M_BKNRI_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SearchData()
    ...
    Private Function BuildSql(searchText As String, ByRef prms As List(Of NpgsqlParameter))
        ...
        sb.AppendLine("  bknri1_cd AS 物件管理単位コード, ")
        sb.AppendLine("  bknri1_nm AS 物件管理単位名, ")
        sb.AppendLine("  bknri2_cd AS 物件管理単位コード2, ")
        sb.AppendLine("  bknri2_nm AS 物件管理単位名2, ")
        sb.AppendLine("  bknri3_cd AS 物件管理単位コード3, ")
        sb.AppendLine("  bknri3_nm AS 物件管理単位名3, ")
        sb.AppendLine("  biko AS 備考, ")
        sb.AppendLine("  create_dt AS 作成日時, ")
        sb.AppendLine("  update_dt AS 更新日時, ")
        sb.AppendLine("  bknri_id AS ID")

        sb.AppendLine("FROM m_bknri ")
        sb.AppendLine("WHERE bknri_id <> 0 ")

        ' --- 検索条件 (WHERE) ---
        If txt_SEARCH.Text.Trim() <> "" Then
            sb.AppendLine("AND bknri1_cd LIKE @search OR bknri2_cd LIKE @search OR bknri3_cd LIKE @search ")
            prms.Add(New NpgsqlParameter("@search", $"%{searchText}%"))
        End If

        sb.AppendLine("ORDER BY bknri_id;")
