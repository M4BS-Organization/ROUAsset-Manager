...
Partial Public Class Form_f_flx_M_GSHA
    ...
    Private Sub Form_f_flx_M_GSHA_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        SearchData()
    End Sub
    ...
    Private Function BuildSql(searchText As String, ByRef prms As List(Of NpgsqlParameter))
        ...
        sb.AppendLine("  gsha_cd AS 業者コード, ")
        sb.AppendLine("  gsha_nm AS 業者名, ")
        sb.AppendLine("  biko AS 備考, ")
        sb.AppendLine("  create_dt AS 作成日時, ")
        sb.AppendLine("  update_dt AS 更新日時, ")
        sb.AppendLine("  gsha_id AS ID")

        sb.AppendLine("FROM m_gsha ")
        sb.AppendLine("WHERE gsha_id <> 0 ")

        ' --- 検索条件 (WHERE) ---
        If txt_SEARCH.Text.Trim() <> "" Then
            sb.AppendLine("AND gsha_cd LIKE @search ")
            prms.Add(New NpgsqlParameter("@search", $"%{searchText}%"))
        End If

        sb.AppendLine("ORDER BY gsha_id")
    ...
    Private Sub cmd_NEW_Click(sender As Object, e As EventArgs) Handles cmd_NEW.Click
        Dim frm As New Form_f_M_GSHA_INP
        frm.ShowDialog()
    ...
    Private Sub cmd_CHANGE_Click(sender As Object, e As EventArgs) Handles cmd_CHANGE.Click
        ...
        Dim frm As New Form_f_M_GSHA_CHANGE
        frm.GshaId = Convert.ToDouble(selectedRow.Cells("id").Value)
        frm.ShowDialog()
