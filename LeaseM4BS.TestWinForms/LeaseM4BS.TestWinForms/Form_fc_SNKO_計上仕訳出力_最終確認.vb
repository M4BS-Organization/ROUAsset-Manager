...
''' <summary>
''' SNKO顧客 計上仕訳出力 最終確認フォーム
''' Access版 fc_SNKO_計上仕訳出力_最終確認 相当
''' tw_fc_swk_wrk（SNKO 計上仕訳データ）を表示し、CSVファイルに出力する。
''' SNKO固有フォーマット: LINE_ID,貸借区分,勘定科目CD,予算組織CD,使用者社員番号,MAPCD,品目CD,金額,消費税CD,消費税額,個別摘要
''' 仮払消費税・雑損失・リース総額仮勘定の固定科目コードを使用。
''' </summary>
Partial Public Class Form_fc_SNKO_計上仕訳出力_最終確認
    Inherits Form
    ...
    Private _karibaraiKamoku As String = ""
    Private _zasonKamoku As String = ""
    Private _sogakuKamoku As String = ""

    Public Sub New()
        InitializeComponent()
    End Sub

    Public Sub SetParams(outputPath As String, keijoBi As String,
                         userEmpNo As String, karibaraiKamoku As String,
                         zasonKamoku As String, sogakuKamoku As String)
        _outputPath = outputPath
        _keijoBi = keijoBi
        _userEmpNo = userEmpNo
        _karibaraiKamoku = karibaraiKamoku
        _zasonKamoku = zasonKamoku
        _sogakuKamoku = sogakuKamoku
    End Sub

    Private Sub Form_fc_SNKO_計上仕訳出力_最終確認_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ' パラメータを画面に反映
    ...
    Private Sub LoadFirstRow()
        Try
            ...
            Dim dtSum = _crud.GetDataTable(
                ...
                "FROM tw_fc_swk_wrk WHERE customer_cd = 'SNKO' AND swk_kbn = '計上仕訳'")
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        Try
            Dim dt = _crud.GetDataTable(
                ...
                "       tekiyo, lsryo, zei, rec_kbn " &
                "FROM tw_fc_swk_wrk " &
                "WHERE customer_cd = 'SNKO' AND swk_kbn = '計上仕訳' ORDER BY den_no, gyo_no")
            ...
            WriteSnkoKeijyoFile(dt, outputPath)
    ...
    ''' <summary>
    ''' SNKOフォーマット（CSV）で計上仕訳ファイル出力する。
    ''' 仮払消費税・雑損失・リース総額仮勘定の固定科目コードを各行に付加する。
    ''' </summary>
    Private Sub WriteSnkoKeijyoFile(dt As DataTable, filePath As String)
        Using sw As New StreamWriter(filePath, False, Encoding.UTF8)
            ...
            For Each row As DataRow In dt.Rows
                ...
                sw.WriteLine(String.Join(",",
                    ...
                    CsvEsc(drHkmCd), "",
                    drKin.ToString("F0"), CsvEsc(_karibaraiKamoku), zeiKin.ToString("F0"),
                    CsvEsc(tekiyo)))
                lineId += 1

                ' 貸方行（リース総額仮勘定または指定科目）
                Dim crKmk = If(String.IsNullOrWhiteSpace(_sogakuKamoku), crKmkCd, _sogakuKamoku)
                sw.WriteLine(String.Join(",",
                    lineId, "2",
                    CsvEsc(crKmk), "", CsvEsc(_userEmpNo),
                    CsvEsc(crHkmCd), "",
                    crKin.ToString("F0"), "", "",
                    CsvEsc(tekiyo)))
                lineId += 1
    ...
    Private Sub Form_fc_SNKO_計上仕訳出力_最終確認_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _crud?.Dispose()
    End Sub
