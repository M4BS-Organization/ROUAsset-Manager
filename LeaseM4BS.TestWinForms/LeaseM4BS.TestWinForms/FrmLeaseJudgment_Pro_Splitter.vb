...
Public Class FrmLeaseJudgment_Pro_Splitter
    Inherits Form
    ...
    Private _isShuttingDown As Boolean = False
        ...
        Private splitMain As SplitContainer           ' ★ 追加：上段/下段のスプリッター
        Private lowerLayout As TableLayoutPanel       ' ★ 追加：下段内の2行（IFRS16・結果）
        ...
        Private Sub InitializeComponent()
            ...
            root.RowStyles.Add(New RowStyle(SizeType.Percent, 100.0F)) ' 1: スプリット（上段/下段）
            root.RowStyles.Add(New RowStyle(SizeType.Absolute, 56.0F)) ' 2: ボタン
            ...
            ' IFRS16 詳細グリッド（Dock=Fill、スクロール無し）
            ...
            ' --- 値コントロール
            numImplicitRate = New NumericUpDown() With {.Minimum = 0, .Maximum = 100, .DecimalPlaces = 2, .Width = 100, .ThousandsSeparator = True, .Dock = DockStyle.Left}
            numIBR = New NumericUpDown() With {.Minimum = 0, .Maximum = 100, .DecimalPlaces = 2, .Width = 100, .ThousandsSeparator = True, .Dock = DockStyle.Left}
            ...
            numExtendMonths = New NumericUpDown() With {.Minimum = 0, .Maximum = 1200, .Width = 90, .Dock = DockStyle.Left}
            ...
            numNonLeaseComp = New NumericUpDown() With {.Maximum = 9999999999D, .ThousandsSeparator = True, .Width = 140, .Dock = DockStyle.Left}

            chkGRVApplicable = New CheckBox() With {.Text = "残価保証適用", .Dock = DockStyle.Left}
            numGRV = New NumericUpDown() With {.Maximum = 9999999999D, .ThousandsSeparator = True, .Width = 140, .Dock = DockStyle.Left}

            numPrepaid = New NumericUpDown() With {.Maximum = 9999999999D, .ThousandsSeparator = True, .Width = 140, .Dock = DockStyle.Left}
            numIDC = New NumericUpDown() With {.Maximum = 9999999999D, .ThousandsSeparator = True, .Width = 140, .Dock = DockStyle.Left}
            numIncentive = New NumericUpDown() With {.Maximum = 9999999999D, .Minimum = -9999999999D, .ThousandsSeparator = True, .Width = 140, .Dock = DockStyle.Left}
            numARO = New NumericUpDown() With {.Maximum = 9999999999D, .ThousandsSeparator = True, .Width = 140, .Dock = DockStyle.Left}
            ...
            ' ===== 下段パネルの2行（IFRS16 / 判定結果） =====
            lowerLayout = New TableLayoutPanel() With {.Dock = DockStyle.Fill, .ColumnCount = 1, .RowCount = 2}
            lowerLayout.RowStyles.Clear()
            lowerLayout.RowStyles.Add(New RowStyle(SizeType.Percent, 75.0F)) ' IFRS16 詳細
            lowerLayout.RowStyles.Add(New RowStyle(SizeType.Percent, 25.0F)) ' 判定結果
            lowerLayout.Controls.Add(grpAdv, 0, 0)
            lowerLayout.Controls.Add(grpResult, 0, 1)
        ...
        ' 上段に top3、下段に lowerLayout を配置
        splitMain.Panel1.Controls.Add(top3)
            splitMain.Panel2.Controls.Add(lowerLayout)

        ' 初期の分割位置（目安）：上 55% / 下 45%
        'splitMain.SplitterDistance = CInt((Me.ClientSize.Height - 40 - 56) * 0.55)
        ...
        Dim btnPanel = New FlowLayoutPanel() With {.Dock = DockStyle.Fill, .FlowDirection = FlowDirection.RightToLeft, .Padding = New Padding(0)}
            ...
            root.Controls.Add(splitMain, 0, 1)   ' ★ 3行構成：タイトル / スプリット / ボタン
        ...
        AddHandler splitMain.SizeChanged, AddressOf OnSplitSizeChanged
    End Sub
    ...
    Private Shared Function MakeLabel(text As String) As Label
        ...
        Private Sub OnFormLoaded(sender As Object, e As EventArgs)
            ' 初回レイアウト後、分割位置を再調整（DPI/描画順に備える）
            If splitMain IsNot Nothing AndAlso splitMain.Height > 0 Then
                Dim avail = splitMain.Height
                Dim target = CInt(avail * 0.55)
                If target > 0 AndAlso Math.Abs(splitMain.SplitterDistance - target) > 12 Then
    ...
    ' 推奨したい見た目の最小高さ（必要に応じて調整）
    Private Const DesiredMin1 As Integer = 220   ' 上段
    Private Const DesiredMin2 As Integer = 220   ' 下段

    ' 表示後に一度だけ、安全に MinSize と SplitterDistance を決める
    Private Sub OnFormShown(sender As Object, e As EventArgs)
        ' レイアウトが確定した直後に実行（BeginInvoke で安定化）
        BeginInvoke(Sub()
                        SetSafePanelMinSizes(DesiredMin1, DesiredMin2)
                        SetSafeSplitterDistance(0.55) ' 上55% / 下45%
                    End Sub)
    End Sub

    ' ウィンドウの高さが変わった時、範囲外ならだけ補正（ユーザーのドラッグは尊重）
    Private Sub OnSplitSizeChanged(sender As Object, e As EventArgs)
        ClampSplitterDistanceIfOutOfRange()
    End Sub

    ' ---- ヘルパ群 ----

    ' 現在の高さに収まるよう、MinSize を安全に決める
    Private Sub SetSafePanelMinSizes(desired1 As Integer, desired2 As Integer)
        If splitMain Is Nothing Then Return
        Dim total As Integer = splitMain.Height
        Dim sw As Integer = splitMain.SplitterWidth
        ...
        Dim maxForMins = total - sw

        ' 希望値の合計が入らない場合は等比で縮小
        Dim dSum = Math.Max(1, desired1 + desired2)
        Dim ratio As Double = Math.Min(1.0, maxForMins / CDbl(dSum))

        Dim m1 = Math.Max(0, CInt(Math.Floor(desired1 * ratio)))
        Dim m2 = Math.Max(0, maxForMins - m1)

        ' いきなり設定すると内部チェックに引っかかることがあるので順に設定
        splitMain.Panel1MinSize = m1
        splitMain.Panel2MinSize = m2
    End Sub

    ' 比率から安全な SplitterDistance を設定（Min～Max にクランプ）
    Private Sub SetSafeSplitterDistance(Optional ratio As Double = 0.55)
        If splitMain Is Nothing Then Return

        Dim total As Integer = splitMain.Height
        Dim min1 As Integer = splitMain.Panel1MinSize
        Dim min2 As Integer = splitMain.Panel2MinSize
        Dim sw As Integer = splitMain.SplitterWidth

        If total <= (min1 + min2 + sw) Then Return

        Dim desired As Integer = CInt(total * ratio)
        Dim minDist As Integer = min1
        Dim maxDist As Integer = total - min2 - sw

        Dim clamped As Integer = Math.Max(minDist, Math.Min(desired, maxDist))
        If clamped <> splitMain.SplitterDistance Then
            splitMain.SplitterDistance = clamped
        End If
    End Sub

    ' 範囲外になっていたらだけ距離を補正
    Private Sub ClampSplitterDistanceIfOutOfRange()
        If splitMain Is Nothing Then Return
        ...
        If splitMain.SplitterDistance < minDist Then
            splitMain.SplitterDistance = minDist
        ElseIf splitMain.SplitterDistance > maxDist Then
            splitMain.SplitterDistance = maxDist
        End If
    End Sub
