...
''' <summary>
''' NKSOL顧客固有 経費仕訳出力フォーム
''' Access版 fc_経費仕訳_NKSOL 相当
''' FcJournalOutputBase を継承し、tw_fc_swk_wrk に経費仕訳を出力して CSV に書き込む。
''' kjkbn_id=3（経費）、rec_kbn IN(1,3) を対象。
''' KAMOKU_CD_01: 出金口座（一括用）、KAMOKU_CD_02: 出金口座（賃借料）
''' KAMOKU_CD_03: 前払費用科目CD、KAMOKU_CD_04: 解約損科目CD
''' chk_CHK_01: 検索条件を加味するフラグ
''' </summary>
Partial Public Class Form_fc_経費仕訳_NKSOL
    ...
    Private Const KEY_SLIP_DT As String = "NKSOL_KEIHI_SLIP_DT"
    Private Const KEY_KAMOKU_01 As String = "NKSOL_KEIHI_KAMOKU_CD_01"
    Private Const KEY_KAMOKU_02 As String = "NKSOL_KEIHI_KAMOKU_CD_02"
    Private Const KEY_KAMOKU_03 As String = "NKSOL_KEIHI_KAMOKU_CD_03"
    Private Const KEY_KAMOKU_04 As String = "NKSOL_KEIHI_KAMOKU_CD_04"
    Private Const KEY_FOLDER1 As String = "NKSOL_KEIHI_FOLDER1"
    Private Const KEY_FILE2 As String = "NKSOL_KEIHI_FILE2"
    ...
    Private Sub Form_fc_経費仕訳_NKSOL_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub cmd_実行_Click(sender As Object, e As EventArgs) Handles cmd_実行.Click
        ...
        If chk_CHK_01.Checked Then
            If MessageBox.Show("経費明細表に検索条件設定がされている場合、データを上書きしますが、よろしいですか？",
        ...
        Dim dtCount = _crud.GetDataTable(
            "SELECT COUNT(*) FROM tw_fc_swk_wrk WHERE customer_cd = 'NKSOL' AND swk_kbn = '経費仕訳'")
        ...
        Dim dt = _crud.GetDataTable(
            "SELECT * FROM tw_fc_swk_wrk WHERE customer_cd = 'NKSOL' AND swk_kbn = '経費仕訳' ORDER BY den_no, gyo_no")
    ...
    ''' <summary>
    ''' tw_s_chuki_keijo → tw_fc_swk_wrk への INSERT SQL（NKSOL経費仕訳版）
    ''' kjkbn_id=3（経費）、rec_kbn IN(1,3)。
    ''' 翌月以降は KAMOKU_CD_03 を cr_kmk_cd として使用。
    ''' </summary>
    Protected Overrides Function BuildInsertToWrkSql(kikanFrom As Date) As String
        ...
        Dim kmkCd03 = txt_KAMOKU_CD_03.Text  ' 前払費用
...
ORDER BY k.kykm_id"
    ...
    Protected Overrides Function WriteOutputFile(dt As DataTable, outputFolder As String) As String
        Dim fileName = Path.Combine(outputFolder, $"NKSOL_経費仕訳_{Format(Now, "yyyyMMdd")}.csv")
    ...
    Private Sub LoadSettings()
        ...
        txt_KAMOKU_CD_02.Text = _settei.GetText(KEY_KAMOKU_02, "1040")
        txt_KAMOKU_CD_03.Text = _settei.GetText(KEY_KAMOKU_03, "0126-0510")
        txt_KAMOKU_CD_04.Text = _settei.GetText(KEY_KAMOKU_04, "1671-9999")
    ...
    Private Sub SaveSettings()
        ...
        _settei.SetText(KEY_KAMOKU_03, txt_KAMOKU_CD_03.Text)
        _settei.SetText(KEY_KAMOKU_04, txt_KAMOKU_CD_04.Text)
        _settei.SetText(KEY_FOLDER1, txt_TEXT_01.Text)
        _settei.SetText(KEY_FILE2, txt_TEXT_02.Text)
    End Sub

    Private Sub Form_fc_経費仕訳_NKSOL_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        _settei?.Dispose()
    End Sub
