...
Public Class FrmContractList
    ...
    Private Sub SearchData()
        ...
        Try
            ...
            ' データ取得＆表示
            Dim dt As DataTable = crud.GetDataTable(sb.ToString(), prms)
            ' ★★★ ここを追加・変更 ★★★
            ...
            End If
            ' 金額を通貨形式に
            FormatColumn("月額リース料", "#,##0")
            FormatColumn("総額リース料", "#,##0")
            ' 日付を短い形式に
            FormatColumn("契約日", "yyyy/MM/dd")
            FormatColumn("開始日", "yyyy/MM/dd")
            FormatColumn("終了日", "yyyy/MM/dd")
    ...
    ' 列フォーマット用ヘルパー
    Private Sub FormatColumn(colName As String, fmt As String)
        If dgv_LIST.Columns.Contains(colName) Then
            dgv_LIST.Columns(colName).DefaultCellStyle.Format = fmt
        End If
    End Sub
    ...
    ' ---------------------------------------------------------
    Private Sub OpenSelectedContract()
        Dim targetRow As DataGridViewRow = Nothing

        ' 1. 「青く選択されている行」を優先して探す
        If dgv_LIST.SelectedRows.Count > 0 Then
            targetRow = dgv_LIST.SelectedRows(0)

            ' 2. なければ「カーソルがある行」を探す
        ElseIf dgv_LIST.CurrentRow IsNot Nothing Then
            targetRow = dgv_LIST.CurrentRow
        End If

        ' 3. それでも取れない、または無効な行（新規行やインデックス異常）ならエラー
        If targetRow Is Nothing OrElse targetRow.Index < 0 Then
            MessageBox.Show("行を選択してください。", "確認", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            Return
        End If

        Try
            ' 選択された行の ID (kykh_id) を取得
            ' ※列名 "kykh_id" が存在するかチェックしても良いですが、SearchDataで作っているので大丈夫なはずです
            Dim selectedId As Double = Convert.ToDouble(targetRow.Cells("kykh_id").Value)

            ' 詳細画面を開く
            Dim frm As New FrmContractEntry()

            ' IDを渡す
            frm.TargetId = selectedId
