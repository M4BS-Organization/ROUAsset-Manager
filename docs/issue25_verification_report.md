...
| # | 問題 | 重要度 | 対応 |
|---|------|--------|------|
| 6 | SecurityCheckerが新規2画面のみ適用 | 中 | 全40画面にApplyXxxLimit()を適用（マスタ21+データ9+帳票12） |
| 7 | シードデータのパスワードが平文 | 中 | 002_seed_dev.sqlの全パスワードをSHA256ハッシュに変更 |
| 8 | ログアウト時の監査ログ | 中 | ApplicationEvents.vbに既に実装済み（WriteAuditLog(OP_LOGOUT)） |
