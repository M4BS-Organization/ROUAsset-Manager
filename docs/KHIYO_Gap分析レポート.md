...
| CHK-016 | 計上タイミング（〆日/約定支払日）ラジオボタン | **部分実装** | UIあり。~~RecBaseプロパティ未設定~~ → **修正済（修正-002）** |
...
| CHK-022 | NextDtTo パラメータ設定 | **部分実装** | ~~未設定~~ → **修正済（修正-001）** `txt_DT_TO.Value.AddMonths(1)` |
| CHK-023 | RecBase（計上タイミング）パラメータ設定 | **部分実装** | ~~未設定~~ → **修正済（修正-002）** |
