...
| CHK-018 | 金額符号（マイナス/プラス）ラジオボタン | **実装済** | サブクエリ化して符号フィルタ適用済（修正-003） |
...
| ID | チェック項目 | 判定 | 備考 |
|---|---|---|---|
| CHK-046 | AddRecConditions() の活用 | **実装済** | デッドコード削除済（修正-006） |
...
| 修正ID | 優先度 | 対象 | 内容 | 状態 |
|---|---|---|---|---|
| 修正-003 | 中 | CHK-018 | 金額符号フィルタをSQLに反映（サブクエリ化して WHERE 期間計上額 < 0 / > 0） | **修正済** |
| 修正-006 | 低 | CHK-046 | デッドコード `AddRecConditions()` の削除（35行） | **修正済** |
